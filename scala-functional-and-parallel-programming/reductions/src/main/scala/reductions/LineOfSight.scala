...
object LineOfSight extends LineOfSightInterface {
  ...
  def lineOfSight(input: Array[Float], output: Array[Float]): Unit = {
    @tailrec
    def lineOfSightRec(idx: Int, prevMax: Float): Unit = {
      if(idx == 0){
        output(idx) = 0
        lineOfSightRec(idx+1, prevMax)
      }else if( idx == input.length){
      }else if(input(idx)/idx > prevMax ){
        output(idx) = input(idx)/idx
        lineOfSightRec(idx+1, input(idx)/idx)
      }else{
        output(idx) = prevMax
        lineOfSightRec(idx+1, prevMax)
      }
    }
    lineOfSightRec(0, 0.0f)
  }
  ...
  def upsweepSequential(input: Array[Float], from: Int, until: Int): Float = {
    ...
    var prevMax = 0.0f
    while(i < until){
      prevMax = Math.max(prevMax, input(i)/i)
      i += 1
    }
    prevMax
  }
