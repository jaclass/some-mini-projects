...
package object barneshut {
  ...
  case class Fork(
    ...
    def insert(b: Body): Fork = {
      if(b.x > centerX + size / 2.0 || b.x < centerX - size / 2.0
        || b.y > centerY + size / 2.0 || b.y < centerY - size / 2.0 )
        this
      else
  ...
  extends Quad {
    val mass = bodies.foldLeft(0f)((f: Float, body: Body) => f + body.mass)
    val massX = if(mass==0) centerX else bodies.foldLeft(0f)((f: Float, body: Body) => f + body.mass * body.x) / mass
    val massY = if(mass==0) centerY else bodies.foldLeft(0f)((f: Float, body: Body) => f + body.mass * body.y) / mass
    ...
    def insert(b: Body): Quad = {
      if(size <= minimumSize)
        ...
        var newFork = Fork(Empty(centerX - delta, centerY - delta, newSize), Empty(centerX + delta, centerY - delta, newSize),
          Empty(centerX - delta, centerY + delta, newSize), Empty(centerX + delta, centerY + delta, newSize)).insert(b)
        bodies.foreach(body => newFork = newFork.insert(body))
        newFork
      }
    }
  }
    ...
    def updated(quad: Quad): Body = {
      ...
      def traverse(quad: Quad): Unit = (quad: Quad) match {
        ...
        case fork @ Fork(nw, ne, sw, se) =>
          if(fork.size / distance(x, y, fork.massX, fork.massY) < theta){
            traverse(fork.nw)
            traverse(fork.ne)
            traverse(fork.sw)
            traverse(fork.se)
