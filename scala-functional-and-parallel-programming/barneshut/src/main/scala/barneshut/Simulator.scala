...
class Simulator(val taskSupport: TaskSupport, val timeStats: TimeStatistics) {
  ...
  def mergeBoundaries(a: Boundaries, b: Boundaries): Boundaries = {
    ...
    newBound.maxX = a.maxX max b.maxX
    newBound.minX = a.minX min b.minX
    newBound.maxY = a.maxY max b.maxY
    newBound.minY = a.minY min b.minY
    newBound
  }
  ...
  def computeSectorMatrix(bodies: coll.Seq[Body], boundaries: Boundaries): SectorMatrix = timeStats.timed("matrix") {
    val parBodies = bodies.par
    parBodies.tasksupport = taskSupport
    parBodies.aggregate(new SectorMatrix(boundaries, SECTOR_PRECISION))((sectorMatrix: SectorMatrix, body: Body) => sectorMatrix += body,
          (sectorMatrixA: SectorMatrix, sectorMatrixB: SectorMatrix) => sectorMatrixA combine sectorMatrixB)
  }
